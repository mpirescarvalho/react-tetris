...
const Pixel = React.memo(styled.div`
  ...
  background-color: ${props =>
	...
	${props => (props.fill && props.theme3d) && `;
		box-shadow: ${props.pixelSize/4.16}px ${props.pixelSize/4.16}px ${props.pixelSize/5.55}px #222${props.topBloco ? `, 0 ${-props.pixelSize/4.16}px 0 ${Color(props.color).lighten(0.2)}` : ''} 
	`};

	${props => !props.theme3d && `
		border-left: 1px solid ${props.stage || props.fill || props.hint ? '#222' : 'black'};
		border-top: 1px solid ${props.stage || props.fill || props.hint ? '#222' : 'black'};	
	`};
		...
		<Game portrait={portrait}>
						...
						{nextRender.map((row, y) => (
							<Row pixelSize={pixelSize} key={`row-${y}`}>
								{row.map((pixel, x) => {
									...
									return (
										<Pixel theme3d={theme3d} topBloco={topBloco} zIndex={y} pixelSize={pixelSize} key={`pixel-${x}`} fill={pixel} color={player.next.color} />
									);
								})}
							</Row>
						))}
			...
			{status && 
				<ContainerStatus portrait={portrait} pixelSize={pixelSize}>
					<StatusRow backgroundColor={theme3d ? '#444' : 'black'} portrait={portrait} borderSize={pixelSize/10} margin={pixelSize/3} padding={pixelSize/2} title='SCORE' value={status.score} />
					<StatusRow backgroundColor={theme3d ? '#444' : 'black'} portrait={portrait} borderSize={pixelSize/10} margin={pixelSize/3} padding={pixelSize/2} title='LEVEL' value={status.level} />
					<StatusRow backgroundColor={theme3d ? '#444' : 'black'} portrait={portrait} borderSize={pixelSize/10} margin={pixelSize/3} padding={pixelSize/2} title='LINES' value={status.lines} />
				</ContainerStatus>}
	</Game>
	);
};
